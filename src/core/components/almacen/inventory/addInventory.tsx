...
const AddInventory = () => {
    ...
    const getDataEdit = async () => {
        ...
        if (response) {
            ...
            setName(`${response.data.nombre} ${response.data.paterno} ${response.data.materno}`)
    ...
    const saveData = async () => {
        ...
        const requestSave = await Api.saveInventario(dataSending)
        if (requestSave) {
            Swal.fire('Inventario Generado', 'El inventario ingresado fue guardado exitosamente.', 'success')
    ...
    return (
        <>
            <Card>
                <CardHeader
                    title="Formulario de Inventario/Almacen"
                    subheader="Registra un nuevo Inventario"
                    avatar={
                    ...
                    <Grid container spacing={2}>
                        <Grid item xs={9}>
                            <Grid container spacing={2}>
                                ...
                                <Grid item xs={8}>
                                    <TextField id="outlined-basic45" value={observation} onChange={(e) => { setObservation(e.target.value) }} label="Descripcion" variant="outlined" fullWidth />
                                </Grid>
                            </Grid>

                        </Grid>
                        <Grid item xs={3} sx={{ textAlign: 'center' }}>
                            <Button variant="contained" startIcon={<AddIcon />} sx={{ mt: 1 }} onClick={() => { handleModal(true) }}>
                                Material
                            </Button>
                        </Grid>
                    </Grid>
            ...
            <LoadingButton loading={load} variant="contained" sx={{ mt: 4, width: 380, marginLeft: 'auto' }} startIcon={<AddIcon />} onClick={saveData} disabled={!observation || cart.length == 0}>
                Guardar
            </LoadingButton>
            ...
            <Dialog onClose={handleModal} open={open} fullScreen={fullScreen}>
                ...
                <DialogContent sx={{ p: 5 }}>
                    <Grid container spacing={2} sx={{ mt: 1 }}>
                        ...
                        <Grid item xs={3}>
                            <TextField id="outlined-basic|1" value={cantidad} label="Cantidad" variant="outlined" onChange={handleInputChange} onBlur={handleBlur} fullWidth />
                        </Grid>
                        <Grid item xs={6}>
                            <TextField id="outlined-basi2c" value={unidad} label="Unidad" variant="outlined" disabled fullWidth />
                        </Grid>
                        <Grid item xs={6}>
                            <TextField id="outlined-basic3" value={marca} label="Marca" variant="outlined" disabled fullWidth />
                        </Grid>
                        <Grid item xs={12}>
                            <TextField id="outlined-basic4" value={observationAdd} onChange={(e) => { setObservationAdd(e.target.value) }} label="Observación" variant="outlined" fullWidth />
                        </Grid>
                        <Grid item xs={6} sx={{ textAlign: 'center' }}>
                            <Button variant="contained" startIcon={<AddIcon />} sx={{ mt: 3 }} disabled={parseInt(cantidad) <= 0} onClick={addCart}>
                                Agregar
                            </Button>
                        </Grid>
                        <Grid item xs={6} sx={{ textAlign: 'center' }}>
